...
global.spr_sys_tri[i]=sprite_add("sy_tmp\"+string(1)+".PNG",0,0,0,0,0)
for(j=2; file_exists("sy_tmp\"+string(j)+".PNG")=1; j+=1)
{
tmp_sprspr=sprite_add("sy_tmp\"+string(j)+".PNG",0,0,0,0,0)
sprite_merge(global.spr_sys_tri[i], tmp_sprspr)
//덧붙이기
}
//스픗불르기

open=file_text_open_read("sy_tmp\name.txt")
global.name_sys_tri[i]=file_text_read_string(open)+file_text_read_string(open)
file_text_close(open)
//이름불르기

global.tri_sys_max_number+=1
//최대 지형수 증가
